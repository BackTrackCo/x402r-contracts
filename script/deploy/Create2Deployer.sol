...
interface ICreateX {
    ...
    function computeCreate2Address(bytes32 salt, bytes32 initCodeHash, address deployer) external view returns (address);
}
...
abstract contract Create2Deployer is Script {
    ...
    function _computeAddress2(string memory label, bytes memory initCode) internal view returns (address) {
        bytes32 salt = bytes32(abi.encodePacked(msg.sender, bytes1(0x00), bytes11(keccak256(bytes(label)))));
        return CREATEX.computeCreate2Address(salt, keccak256(initCode), address(CREATEX));
    }
}
