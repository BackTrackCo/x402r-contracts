...
### Arbiter
An external address (e.g., dispute resolution service) that can approve/deny refund requests when funds are in escrow. Authorized via the operator's `VOID_CONDITION` slot.
...
### Condition Slot
One of 5 immutable condition addresses on a PaymentOperator: `AUTHORIZE_CONDITION`, `CHARGE_CONDITION`, `CAPTURE_CONDITION`, `VOID_CONDITION`, `REFUND_CONDITION`. `address(0)` means "allow all" (default).
...
### Hook Slot
One of 5 immutable hook addresses on a PaymentOperator: `AUTHORIZE_HOOK`, `CHARGE_HOOK`, `CAPTURE_HOOK`, `VOID_HOOK`, `REFUND_HOOK`. `address(0)` means "no-op" (default).

### HookCombinator
Composes multiple hooks into a single slot. Calls each sub-hook sequentially. Limited to `MAX_HOOKS = 10`.
...
### Escrow Period
A time window after authorization during which funds are held in escrow before capture is permitted. Enforced by `EscrowPeriod` (which implements both `ICondition` and `IHook`) as both the `AUTHORIZE_HOOK` and `CAPTURE_CONDITION`.
...
### RefundInEscrow
Refund path while funds are still in escrow (capturable > 0). Reduces `capturableAmount` and increases `refundableAmount`. Governed by `VOID_CONDITION`.

### RefundPostEscrow
Refund path after funds are captured (capturable = 0). Receiver must voluntarily refund from their own balance via the ReceiverRefundCollector. Governed by `REFUND_CONDITION`.
