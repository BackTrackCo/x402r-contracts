...
contract Escrow is EscrowAccess {
    ...
    IERC4626 public immutable VAULT;   // Hardcoded vault for all deposits
        ...
        returns (uint256 depositNonce)
    {
        ...
        require(TOKEN.allowance(address(this), address(VAULT)) >= amount, "Escrow: Insufficient allowance");
    ...
    function release(address user, uint256 depositNonce) external {
        ...
        _withdrawAndDistribute(merchantPayout, principalAmount, yieldAmount, merchantPayout);
        
        emit DepositReleased(user, merchantPayout, principalAmount, yieldAmount, depositNonce);
    }
        ...
        onlyMerchantOrArbiter(deposits[user][depositNonce].merchantPayout)
    {
        ...
        _clearDeposit(user, depositNonce);
        _withdrawAndDistribute(user, principalAmount, yieldAmount, deposit.merchantPayout);
        
        emit DepositRefunded(user, msg.sender, principalAmount, yieldAmount, depositNonce);
    }
}
