...
import {StaticAddressPreActionCondition} from "./StaticAddressPreActionCondition.sol";

/**
 * @title StaticAddressPreActionConditionFactory
 * @notice Factory for deploying StaticAddressPreActionCondition instances with deterministic addresses.
 ...
 */
contract StaticAddressPreActionConditionFactory {
    error ZeroAddress();
    ...
    event StaticAddressPreActionConditionDeployed(address indexed condition, address indexed designatedAddress);

    /**
     * @notice Deploy a new StaticAddressPreActionCondition
    ...
    function deploy(address designatedAddress) external returns (address condition) {
        ...
        bytes memory bytecode =
            abi.encodePacked(type(StaticAddressPreActionCondition).creationCode, abi.encode(designatedAddress));
        condition = address(
        ...
        emit StaticAddressPreActionConditionDeployed(condition, designatedAddress);

        // ============ INTERACTIONS ============
        address deployed = address(new StaticAddressPreActionCondition{salt: salt}(designatedAddress));
