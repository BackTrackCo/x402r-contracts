...
import {SignaturePreActionCondition} from "./SignaturePreActionCondition.sol";

/**
 * @title SignaturePreActionConditionFactory
 * @notice Factory for deploying SignaturePreActionCondition instances with deterministic addresses.
 *         Uses CREATE2 for address predictability.
 *
 * @dev Key is keccak256(signer). Each unique signer gets one canonical deployment.
 */
contract SignaturePreActionConditionFactory {
    error ZeroSigner();
    ...
    event SignaturePreActionConditionDeployed(address indexed condition, address indexed signer);

    /**
     * @notice Deploy a new SignaturePreActionCondition for a signer
    ...
    function deploy(address signer) external returns (address condition) {
        ...
        bytes memory bytecode = abi.encodePacked(type(SignaturePreActionCondition).creationCode, abi.encode(signer));
        condition = address(
        ...
        emit SignaturePreActionConditionDeployed(condition, signer);

        // ============ INTERACTIONS ============
        address deployed = address(new SignaturePreActionCondition{salt: salt}(signer));
