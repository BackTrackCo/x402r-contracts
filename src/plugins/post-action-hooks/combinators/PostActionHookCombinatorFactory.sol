...
contract PostActionHookCombinatorFactory {
    ...
    /// @notice Maximum hooks allowed (matches PostActionHookCombinator.MAX_POST_ACTION_HOOKS)
    ...
    event PostActionHookCombinatorDeployed(address indexed combinator, IPostActionHook[] hooks);
     ...
     */
    function deploy(IPostActionHook[] calldata _hooks) external returns (address combinator) {
        ...
        bytes memory bytecode = abi.encodePacked(type(PostActionHookCombinator).creationCode, abi.encode(_hooks));
        combinator = address(
        ...
        emit PostActionHookCombinatorDeployed(combinator, _hooks);

        // ============ INTERACTIONS ============
        address deployed = address(new PostActionHookCombinator{salt: salt}(_hooks));
     ...
     */
    function getDeployed(IPostActionHook[] calldata _hooks) external view returns (address combinator) {
     ...
     */
    function computeAddress(IPostActionHook[] calldata _hooks) external view returns (address combinator) {
        bytes32 key = getKey(_hooks);
        bytes32 salt = keccak256(abi.encodePacked("hookCombinator", key));
        bytes memory bytecode = abi.encodePacked(type(PostActionHookCombinator).creationCode, abi.encode(_hooks));
     ...
     */
    function getKey(IPostActionHook[] calldata _hooks) public pure returns (bytes32) {
